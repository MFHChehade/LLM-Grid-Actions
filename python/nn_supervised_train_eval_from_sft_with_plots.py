...
# ---------------------------
# Publishable plot style (training curves)
# ---------------------------

FIG_DPI = 600

plt.rcParams.update({
    # Serif + embedded PDF fonts (avoid Type-3)
    "font.family": "serif",
...
def infer_indexing_from_many(open_ids_lists: List[List[int]]) -> str:
    """
    Infer indexing consistently across a split.
    If ANY open id is 0 => treat as 0-based, else 1-based.
    """
    for ids in open_ids_lists:
...
def build_examples(io_dir: Path, raw_rows: List[Dict], split_ids: List[str],
    ...
    Improvements:
      - infer indexing ONCE for the whole split (not per-row)
      - keep split deterministic (sorted)
    """
    ...
    # First pass: load xi, parse open_ids, cache prompt/case/k/gt, infer n_line
    n_line: Optional[int] = None
    cache: Dict[str, Dict] = {}
    open_ids_lists: List[List[int]] = []
    ...
    for sid in split_ids:
        ...
        open_ids_lists.append(open_ids)
        ...
        try:
            ...
            if isinstance(prompt_obj, dict):
                if "toggle_budget" in prompt_obj:
    ...
    indexing = infer_indexing_from_many(open_ids_lists)

    # Second pass: build examples with consistent indexing
    examples: List[Example] = []
    for sid in split_ids:
        c = cache[sid]
        y = make_multi_hot(c["open_ids"], n_line, indexing=indexing)
        examples.append(Example(
            ...
            target_text=str(c["target_text"]),
            gt_file=c["gt_file"],
        ))

    return examples, indexing, int(n_line)
...
def decode_topk(logits: torch.Tensor, xi: torch.Tensor, k: int, indexing: str) -> List[int]:
    ...
    Fix:
      - cap k by number of eligible lines (avoid selecting masked -1e30 entries)
    """
    with torch.no_grad():
...
def run_matlab_verify(repo_root: Path, case_name: str, plan_json: Path, xi_json: Path,
    ...
    IMPORTANT (Windows): avoid backslashes inside f-string expression parts.
    We normalize paths BEFORE formatting the MATLAB command string.
    """
    matlab_dir = repo_root / "matlab"
    fn = "verify_plan_hybrid" if hybrid else "verify_plan"

    matlab_dir_s = str(matlab_dir).replace("\\", "/")
    plan_s = str(plan_json).replace("\\", "/")
    xi_s = str(xi_json).replace("\\", "/")
    limits_s = str(limits_yml).replace("\\", "/")
    out_s = str(out_json).replace("\\", "/")
...
def _savefig(fig, fig_dir: Path, name: str, dpi: int = FIG_DPI) -> None:
...
def _label_line_end(ax, x, y, text: str, dx: float = 0.02, dy: float = 0.0):
    """Label at the last finite point."""
    ...
    if not mask.any():
        return
    i = np.where(mask)[0][-1]
    ax.text(xx[i] + dx * (xx.max() - xx.min() + 1e-9), yy[i] + dy, text,
            fontsize=15, va="center", ha="left")


def save_training_plots(train_log: list, fig_dir: Path, suffix: str = "") -> None:
    """
    Publishable training curves (no titles, no color dependence):
      - train loss vs epoch
      - test metrics (jaccard, f1, exact) vs epoch (direct labels, no legend)
      - train vs test jaccard vs epoch (direct labels, no legend)

    Styling:
      - big fonts, thin lines, light grid, no top/right spines
      - best epoch marked with a thin dashed vertical line
    """
    if not train_log:
        return
    ...
    epochs = np.array([int(r.get("epoch", i + 1)) for i, r in enumerate(train_log)], dtype=int)
    loss   = np.array([float(r.get("loss", float("nan"))) for r in train_log], dtype=float)

    def _metric(split: str, key: str):
        out = []
        for r in train_log:
            d = r.get(split, {}) or {}
    ...
    tr_jacc = _metric("train", "jaccard")
    te_jacc = _metric("test",  "jaccard")
    te_f1   = _metric("test",  "f1")
    te_ex   = _metric("test",  "exact")

    # best epoch: same key used for checkpointing
    keys = []
    for r in train_log:
        tm = r.get("test", {}) or {}
        k = float(tm.get("jaccard", 0.0)) + 1e-3 * float(tm.get("f1", 0.0))
        keys.append(k)
    best_epoch = int(epochs[int(np.argmax(keys))]) if keys else None

    def _vline_best(ax):
        if best_epoch is not None:
            ax.axvline(best_epoch, linestyle="--", linewidth=0.9, color="black", alpha=0.75)

    # 1) Loss
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(epochs, loss, linestyle="-", color="black")
    _vline_best(ax)
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Train loss")
    _format_pub_ax(ax)
    fig.tight_layout(pad=0.25)
    _savefig(fig, fig_dir, f"curve_loss{suffix}")
    plt.close(fig)

    # 2) Test metrics (no legend, direct labels; distinguish by linestyle)
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(epochs, te_jacc, linestyle="-",  color="black")
    ax.plot(epochs, te_f1,   linestyle="--", color="black")
    ax.plot(epochs, te_ex,   linestyle=":",  color="black")
    _vline_best(ax)
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Metric")
    _format_pub_ax(ax)

    _label_line_end(ax, epochs, te_jacc, "test jaccard", dy=0.0)
    _label_line_end(ax, epochs, te_f1,   "test f1",      dy=0.0)
    _label_line_end(ax, epochs, te_ex,   "test exact",   dy=0.0)

    fig.tight_layout(pad=0.25)
    _savefig(fig, fig_dir, f"curve_test_metrics{suffix}")
    plt.close(fig)

    # 3) Train vs test jaccard (no legend, direct labels)
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(epochs, tr_jacc, linestyle="-",  color="black")
    ax.plot(epochs, te_jacc, linestyle="--", color="black")
    _vline_best(ax)
    ax.set_xlabel("Epoch")
    ...
    _label_line_end(ax, epochs, tr_jacc, "train", dy=0.0)
    _label_line_end(ax, epochs, te_jacc, "test",  dy=0.0)

    fig.tight_layout(pad=0.25)
    _savefig(fig, fig_dir, f"curve_jaccard_train_vs_test{suffix}")
    plt.close(fig)
...
def main() -> None:
    ...
    ap.add_argument("--fig_dir", type=str, default=None, help="Directory to save training curves (PNG+PDF). Default: io/figs_nn_train")
    ap.add_argument("--no_plots", action="store_true", help="Disable saving training curve figures.")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == args.epochs:
            print(
                f"epoch {epoch:4d}/{args.epochs} | loss {epoch_loss:.4f} | "
    ...
    # Save training curves (PNG+PDF)
    if not args.no_plots:
        fig_dir = Path(args.fig_dir) if args.fig_dir else (io_dir / "figs_nn_train")
        save_training_plots(train_log, fig_dir, suffix)
        print(f"[OK] wrote training figures: {fig_dir}")
    ...
    for idx, r in enumerate(per_rows):
        ...
        if do_matlab and limits_yml.exists():
            if matlab_cap <= 0 or idx < matlab_cap:
                ...
                # NOTE: sid is the xi basename (psps_*.json), stored in io/
                xi_path = io_dir / basename_any(ex.sid)
    ...
    with pred_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for r in rows_out:
            rr = dict(r)
            rr["pred_ids"] = json.dumps(rr["pred_ids"])
            rr["true_ids"] = json.dumps(rr["true_ids"])
            rr["gt_opt_switches"] = json.dumps(rr["gt_opt_switches"]) if isinstance(rr["gt_opt_switches"], (list, dict)) else str(rr["gt_opt_switches"])
            w.writerow(rr)
